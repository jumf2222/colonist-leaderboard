...
function LineChart(props: {
  ...
  const scaleX = (x: number) =>
    padding.left + (bounds().maxX > 0 ? (x / bounds().maxX) * innerW : 0);
  ...
  return (
    <div class="chart-container">
      <svg viewBox={`0 0 ${width} ${height}`} class="chart-svg">
        ...
        {/* X axis label */}
        <text x={padding.left + innerW / 2} y={height - 4} class="chart-label" text-anchor="middle">
          Game #
        </text>
        ...
        <For each={props.series}>
          {(series) => {
            const dimmed = () =>
              props.highlightedPlayer != null && series().name !== props.highlightedPlayer;
            return (
              <path
        ...
        <Show when={hoveredIdx() !== null}>
          <For each={props.series}>
            {(series) => {
              ...
              return (
                <Show when={pt()}>
                  <circle cx={scaleX(pt().x)} cy={scaleY(pt().y)} r="4" fill={series().color} />
                </Show>
              );
            }}
          </For>
    ...
    <div class="overview">
      <div class="overview-grid">
        ...
        <div class="chart-card">
          ...
          <LineChart series={vpOverTime()} yLabel="VP" highlightedPlayer={highlightedPlayer()} />
        </div>
            ...
            <For each={props.leaderboard.players}>
              {(player, pi) => (
                <tr
                  ...
                  class={[
                    "overview-stats-row",
                    { "overview-stats-row-active": highlightedPlayer() === player().username },
                  ]}
                >
                  <td class="overview-player-cell">
                    <span
                      class="overview-player-dot"
